...
// let age = 50;
// let rst = age ? "adult" : "old";
// console.log(rst);

// let name1 = 0;
// let fullName = name1 ?? "this is name";
...
// const data = people.products.flatMap((s) =>
//   s.reviews.filter((a) => a.rating > 2)
// );

// console.log(data);

// const exData = [];
// for (let i = 0; i < people.products.length; i++) {
//   exData.push(people.products[i].reviews.filter((w) => w.rating % 2 !== 0));
// }
// console.log(exData);
...
// let exTems = items.filter((s) => s.data?.price > 120 && s.data.price < 500);
// console.log(exTems);
// let jsonData = items.filter((d) => d.data?.price ?? 0);

// let jsonData = items.filter((e) => e.data?.price ?? "no price" !== "no price");

// console.log(jsonData);

// let exData = items.filter((d) => d.data?.price > 100 || d.data?.price < 50);
...
// let rst = obj.address.map((e) => e.mandal?.length ?? "not mandal");
// console.log(rst);

// const user = { name: "Bob" };
// console.log(user.address?.city); // undefined (no error)

// let objects = obj.salary?.length || "no name";
// console.log(objects);
...
let results = exObject?.salary?.length ?? "not salary available";
console.log(results);
...
let exArray = [
  { id: 123, name: "priya", occupation: "software" },
  { id: 12, occupation: "software" },
];

let ss = exArray.map((w) => w.salary?.length ?? "no name");
console.log(ss);
...
// let data = exData.filter((s) => s.name === "Apple iPad Air");

// console.log(data);
let data = exData.map((s) => s.data?.Capacity ?? "no capacity");
console.log(data);

let exObject1 = null;
let data1 = exObject1 || "no data";
console.log(data1);
